...
class JobRepository:
    ...
    def create(self, job: Job) -> int:
        def operation(conn: sqlite3.Connection) -> int:
            cursor = conn.execute(
                "INSERT INTO jobs(job_type,state,account_id,source_dataset_id,target_group_id,total) VALUES(?,?,?,?,?,?)",
                (
                    str(job.job_type),
                    str(job.state),
                    job.account_id,
    ...
    def list_recent(self, limit: int = 200):
        with self.db.reader() as conn:
            rows = conn.execute(
                "SELECT * FROM jobs ORDER BY id DESC LIMIT ?", (max(1, min(1000, limit)),)
            ).fetchall()
    ...
    def has_nonterminal_jobs(self, account_id: int) -> bool:
        with self.db.reader() as conn:
            row = conn.execute(
                "SELECT 1 FROM jobs WHERE account_id=? AND state NOT IN ('COMPLETED','COMPLETED_WITH_ERRORS','FAILED','CANCELLED') LIMIT 1",
                (account_id,),
            ).fetchone()
        return bool(row)
    ...
    def account_waiting_until(self, account_id: int) -> str | None:
        with self.db.reader() as conn:
            row = conn.execute(
                """SELECT waiting_until FROM jobs
                   WHERE account_id=?
                     AND state IN ('WAITING_SERVER','PAUSED')
                     AND waiting_until IS NOT NULL
                   ORDER BY waiting_until DESC LIMIT 1""",
                (account_id,),
            ).fetchone()
    ...
    def submit_set_state(
        ...
        *,
        ...
        def operation(conn):
            ...
            params = [str(state)]
            if waiting_until is not None or clear_waiting:
                sets.append("waiting_until=?")
                params.append(waiting_until)
    ...
    def checkpoint(self, job_id: int, checkpoint: dict) -> None:
        self.submit_checkpoint(job_id, checkpoint).result(timeout=10)

    def submit_add_items(self, job_id: int, items: list[tuple[int, str]]) -> Future:
        def operation(conn):
            conn.executemany(
                "INSERT OR IGNORE INTO migration_items(job_id,ordinal,member_id,target_state) VALUES(?,?,?,?)",
                [(job_id, i, member_id, state) for i, (member_id, state) in enumerate(items)],
            )
            conn.execute("UPDATE jobs SET total=? WHERE id=?", (len(items), job_id))
    ...
    def pending_chunk(self, job_id: int, account_id: int, last_ordinal: int, limit: int):
        with self.db.reader() as conn:
            rows = conn.execute(
                """SELECT mi.*,m.telegram_user_id,m.username,m.first_name,m.last_name,m.phone,
                          m.bot,m.deleted,m.activity_status,m.last_seen,pc.access_hash account_access_hash
                   ...
                   WHERE mi.job_id=? AND mi.ordinal>? AND mi.state IN ('READY','RETRY')
                   ORDER BY mi.ordinal LIMIT ?""",
                (account_id, job_id, last_ordinal, limit),
            ).fetchall()
        return [dict(row) for row in rows]

    def submit_update_items_batch(
        self, job_id: int, updates: list[ItemUpdate], checkpoint: dict | None = None
    ) -> Future:
        def operation(conn):
            for update in updates:
                conn.execute(
                    """UPDATE migration_items
                       SET state=?,attempt_count=?,last_error_code=?,last_error_text=?,processed_at=CURRENT_TIMESTAMP
            ...
            counts = conn.execute(
                """SELECT SUM(state='SUCCESS') success,SUM(state='SKIPPED') skipped,
            ...
            conn.execute(
                """UPDATE jobs SET processed=?,success=?,skipped=?,failed=?,
    ...
    def submit_mark_retry(
        ...
        return self.writer.execute(
            """UPDATE migration_items SET state='RETRY',attempt_count=?,last_error_code=?,
                      last_error_text=?,next_retry_at=? WHERE job_id=? AND ordinal=?""",
            (attempts, error_code, error_text, next_retry_at, job_id, ordinal),
            critical=True,
        )
    ...
    def submit_event(
        ...
        return self.writer.execute(
            "INSERT INTO job_events(job_id,level,event_code,member_id,message) VALUES(?,?,?,?,?)",
            (job_id, level, event_code, member_id, message),
            critical=critical,
        )

    def summary(self, job_id: int) -> dict:
        row = self.get(job_id) or {}
        return {key: int(row.get(key, 0) or 0) for key in ("total", "processed", "success", "skipped", "failed")}

    def get_checkpoint(self, job_id: int) -> dict:
        row = self.get(job_id)
        raw = row.get("checkpoint_json") if row else None
        try:
    ...
    def recoverable(self) -> list[int]:
        with self.db.reader() as conn:
            rows = conn.execute(
                "SELECT id FROM jobs WHERE state IN ('RUNNING','WAITING_SERVER','PAUSED','RATE_LIMITED') ORDER BY id"
            ).fetchall()
        ...
        with self.db.reader() as conn:
            rows = conn.execute(
                """SELECT DISTINCT m.telegram_user_id
                   FROM jobs j
                   JOIN migration_items mi ON mi.job_id=j.id
                   JOIN members m ON m.id=mi.member_id
                   WHERE j.target_group_id=? AND mi.state='SUCCESS'
